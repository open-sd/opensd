...
bool FileMgr::IsLocalBuild()
{
    ...
    path = "data/config/" + Config::DEFAULT_CONFIG_FILENAME;
    if ((!fs::exists( path )) || (!fs::is_regular_file( path )))
    {
        gLog.Write( Log::DEBUG, FUNC_NAME, "File '" + path.string() + "' does not exist." );
        return false;
    ...
    path = "data/profiles/" + Config::DEFAULT_PROFILE_FILENAME;
    if ((!fs::exists( path )) || (!fs::is_regular_file( path )))
    {
        gLog.Write( Log::DEBUG, FUNC_NAME, "File '" + path.string() + "' does not exist." );
        return false;
...
bool FileMgr::HasSystemConfig()
{
    ...
    path = Config::SYSTEM_CONFIG_DIR + Config::DEFAULT_CONFIG_FILENAME;
    if ((!fs::exists( path )) || (!fs::is_regular_file( path )))
    {
        gLog.Write( Log::DEBUG, FUNC_NAME, "File '" + path.string() + "' does not exist." );
        return false;
    ...
    path = Config::SYSTEM_PROFILE_DIR;
    if ((!fs::exists( path )) || (!fs::is_directory( path )))
    {
        gLog.Write( Log::DEBUG, FUNC_NAME, "Directory '" + path.string() + "' does not exist." );
        return false;
...
int FileMgr::CopyUserConfigFile()
{
    ...
    src = mDataDir.string() + "config/" + Config::DEFAULT_CONFIG_FILENAME;
    dst = mConfigDir.string() + Config::DEFAULT_CONFIG_FILENAME;
...
int FileMgr::Init()
{
    ...
    if (IsInstalled())
    {
        gLog.Write( Log::INFO, "Detected OpenSD installation in '" + Config::INSTALL_DATA_DIR + "'" );
        mDataDir = Config::INSTALL_DATA_DIR;
    }
    else
    {
    ...
    if (HasUserHome())
    {
        ...
        else
        {
            ...
            path = mConfigDir.string() + Config::DEFAULT_CONFIG_FILENAME;
            if (!fs::exists( path ))
            {
                result = CopyUserConfigFile();
                if (result != Err::OK)
    ...
    else
    {
        ...
        else
        {
            ...
            mConfigDir  = Config::SYSTEM_CONFIG_DIR;
            mProfileDir = Config::SYSTEM_PROFILE_DIR;
